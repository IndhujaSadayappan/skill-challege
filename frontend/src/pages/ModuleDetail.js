...
const ModuleDetail = () => {
  ...
  const fetchModule = async () => {
    try {
      setLoading(true)
      const response = await fetch(`${"http://13.232.214.235:5000"}/api/skills/${skillId}/modules/${moduleId}`, {  // Updated endpoint
        credentials: "include",
      })
  ...
  const updateProgress = async (lessonId, completed, score = null) => {
    try {
      await fetch(`${"http://13.232.214.235:5000"}/api/skills/${skillId}/progress`, {  // Updated for skills
        method: "POST",
        credentials: "include",
        headers: {
