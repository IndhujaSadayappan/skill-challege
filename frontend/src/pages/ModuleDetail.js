...
const ModuleDetail = () => {
  ...
  const fetchModule = async () => {
    try {
      setLoading(true)
      const response = await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/skills/${skillId}/modules/${moduleId}`, {  // Updated endpoint
        credentials: "include",
      })
  ...
  const updateProgress = async (lessonId, completed, score = null) => {
    try {
      await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/skills/${skillId}/progress`, {  // Updated for skills
        method: "POST",
        credentials: "include",
        headers: {
