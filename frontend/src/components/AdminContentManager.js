...
const AdminContentManager = () => {
  ...
  const fetchCourses = async () => {
    try {
      setLoading(true)
      const response = await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/admin/courses`, {
        credentials: "include",
      })
  ...
  const handleAddModule = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/admin/courses/${selectedCourse._id}/modules`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  const handleAddLesson = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/admin/courses/${selectedCourse._id}/modules/${selectedModule._id}/lessons`,
        {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteCourse = async (courseId) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/admin/courses/${courseId}`, {
        method: "DELETE",
        credentials: "include",
      })
